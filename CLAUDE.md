...
- **Set Healthcheck Path = `/up`** (Rails' `rails/health#show`) in Railway Deploy settings: Railway then waits for the new container to pass before cutting traffic, and keeps the old deploy serving if a new one crashes (zero-downtime + no 502 from a bad deploy).
